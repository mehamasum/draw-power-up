...
var AboutDialog = function(editorUi)
{
	...
	mxUtils.write(h3, mxResources.get('about') + ' Draw in Trello');
	div.appendChild(h3);
	var img = document.createElement('img');
	img.style.border = '0px';
	img.setAttribute('width', '176');
	img.setAttribute('width', '151');
	img.setAttribute('src', './powerup/images/icon.svg');
	div.appendChild(img);
	mxUtils.br(div);
	mxUtils.write(div, 'Powered by mxGraph ' + mxClient.VERSION);
	mxUtils.br(div);
	var link = document.createElement('a');
	link.setAttribute('href', 'https://github.com/mehamasum/draw-power-up');
	link.setAttribute('target', '_blank');
	mxUtils.write(link, 'Fork on Github');
